...
func handlePptxSingle(args []string) {
	if len(args) < 8 {
		fmt.Println("Error: Missing required arguments")
		fmt.Println("\nUsage:")
		fmt.Println("  officeforge pptx-single --input <template> --output <file> --key <keyword> --value <replacement>")
		os.Exit(1)
	}
	...
	err := pptx.ProcessPptxSingle(inputPath, outputPath, keyword, replacement)
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		os.Exit(1)
	}
...
func handlePptxMulti(args []string) {
	if len(args) < 6 {
		fmt.Println("Error: Missing required arguments")
		fmt.Println("\nUsage:")
		fmt.Println("  officeforge pptx-multi --input <template> --output <file> --data <json_file>")
		os.Exit(1)
	}
	...
	err = pptx.ProcessPptxMulti(inputPath, outputPath, replacements)
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		os.Exit(1)
	}
...
func handlePptxBatch(args []string) {
	if len(args) < 6 {
		fmt.Println("Error: Missing required arguments")
		fmt.Println("\nUsage:")
	...
	// Process presentations using the pattern
	err = pptx.ProcessPptxMultipleRecords(inputPath, outputDir, records, pattern)
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		os.Exit(1)
	}

	fmt.Printf("✓ Generated %d presentations in: %s\n", len(records), outputDir)
}

func handlePptxCheck(args []string) {
	if len(args) < 4 {
		fmt.Println("Error: Missing required arguments")
		fmt.Println("\nUsage:")
		fmt.Println("  officeforge pptx-check --input <file> [--keys \"K1,K2\" | --data <json_file>] [--json]")
		os.Exit(1)
	}
	...
	// Call the validation function (you'll need to implement ValidatePptxKeywords in internal)
	results, err := internal.ValidatePptxKeywords(inputPath, keywords)
	if err != nil {
		if outputJson {
